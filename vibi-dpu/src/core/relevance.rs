...
use crate::{
	bitbucket::{self, user::author_from_commit},
	core::github,
	db::review::save_review_to_db,
	utils::{
		aliases::get_login_handles,
...
pub async fn process_relevance(
	...
	access_token: &str,
	old_review_opt: &Option<Review>,
) {
	log::info!(
		"[process_relevance] Process relevence for PR: {}, repo config: {:?}",
		review.id(),
		repo_config
	);
	log::debug!(
		"[process_relevance] relevance vec = {:?}",
		old_review_opt);
	for prhunk in hunkmap.prhunkvec() {
		...
		if repo_config.auto_assign() {
			...
			log::debug!(
				"[process_relevance] review.provider() = {:?}",
				review.provider()
			);
			if review.provider().to_string() == ProviderEnum::Bitbucket.to_string() {
...
fn compare_relevance_vectors(relevance_new: &[Relevance], relevance_old: &[Relevance]) -> bool {
    ...
    for relevance_item_new in relevance_new {
        ...
        if !found_match {
			log::debug!(
				"[compare_relevance_vectors] not found match for {} in old",
				relevance_item_new.git_alias());
            return false;
        }
    }
	log::debug!(
		"[compare_relevance_vectors] all relevance vectors are same");
...
async fn add_bitbucket_reviewers(
	prhunk: &PrHunkItem,
	hunkmap: &HunkMap,
	review: &Review,
	access_token: &str,
	...
	for blame in prhunk.blamevec() {
		let blame_author_opt =
			author_from_commit(blame.commit(), hunkmap.repo_name(), hunkmap.repo_owner()).await;
		if blame_author_opt.is_none() {
			log::error!(
				"[process_relevance] Unable to get blame author from bb for commit: {}",
				&blame.commit()
			);
			continue;
		}
...
async fn calculate_relevance(prhunk: &PrHunkItem, review: &mut Review) -> Option<Vec<Relevance>> {
	...
	for blame in prhunk.blamevec() {
		...
		let num_lines: f32 = blame
			.line_end()
			.parse::<f32>()
			.expect("lines_end invalid float")
			- blame
				.line_start()
				.parse::<f32>()
				.expect("lines_end invalid float")
		...
		if relevance_floatmap.contains_key(&author_id) {
			let relevance = relevance_floatmap
				.get(&author_id)
				.expect("unable to find coverage for author")
				+ num_lines;
...
fn comment_text(relevance_vec: &Vec<Relevance>, auto_assign: bool) -> String {
	...
	let (deduplicated_relevance_map, unmapped_aliases) =
		deduplicated_relevance_vec_for_comment(relevance_vec);
	let mut deduplicated_relevance_vec: Vec<(&Vec<String>, &f32)> =
		deduplicated_relevance_map.iter().collect();
	deduplicated_relevance_vec
		.sort_by(|(_, a), (_, b)| b.partial_cmp(a).unwrap_or(std::cmp::Ordering::Equal)); // I couldn't find a way to avoid unwrap here :(
...
pub fn deduplicated_relevance_vec_for_comment(
	relevance_vec: &Vec<Relevance>,
) -> (HashMap<Vec<String>, f32>, Vec<String>) {
