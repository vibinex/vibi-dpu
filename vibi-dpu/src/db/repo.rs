...
pub fn save_repo_to_db(repo: &Repository) {
	...
	if parse_res.is_err() {
		...
		eprintln!("Unable to serialize repo: {:?}, error: {:?}", repo, e);
		return;
	}
    ...
    if insert_res.is_err() {
        ...
        eprintln!("Failed to upsert repo into sled DB: {:?}", e);
        return;
    }
    println!("Repo succesfully upserted: {:?}", repo);
}
...
pub fn get_clone_url_clone_dir(repo_provider: &str, workspace_name: &str, repo_name: &str) -> Option<(String, String)> {
	...
	if repo_opt.is_none() {
		eprintln!("Empty repo_opt from db");
		return None;
	}
	...
	if parse_res.is_err() {
		...
		eprintln!("error in deserializing repo from db: {:?}", e);
		return None;
	}
	...
	if clone_dir_opt.is_none() {
		eprintln!("Empty clone_dir_opt in db, repo: {:?}", &repo);
		return None;
	}
