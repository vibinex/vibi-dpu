...
pub async fn get_user_github_repos_using_graphql_api(
    ...
    let query = format!("query {{ viewer {{ repositories(first: {pageCount}, affiliations: [OWNER, ORGANIZATION_MEMBER, COLLABORATOR], ownerAffiliations: [OWNER, ORGANIZATION_MEMBER, COLLABORATOR]) {{ totalCount nodes {{ name id isPrivate sshUrl owner {{ login }} }} }} }} }}");
    let body = json!({
        "query": query
    });
